...
public interface UserDAO {
     ...
     */
    @Insert("INSERT INTO hk_users (user_id, username, email, phone, password, profile_image, birth_date, gender, status) " +
            "VALUES (#{user_id}, #{username}, #{user_id}, #{phone}, #{password}, #{profile_image, jdbcType=VARCHAR}, #{birth_date, jdbcType=DATE}, #{gender, jdbcType=VARCHAR}, #{status, jdbcType=VARCHAR})")
    ...
    @Select("SELECT * FROM hk_users WHERE user_id = #{user_id} AND password = #{password}")
    ...
    @Select("SELECT * FROM hk_users WHERE user_id = #{user_id}")
